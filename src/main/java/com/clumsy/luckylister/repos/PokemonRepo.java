...
public interface PokemonRepo extends JpaRepository<PokemonEntity, Long> {
	...
	@Query("SELECT t FROM PokemonEntity t WHERE t.lucky=true ORDER BY t.dexid ASC, t.variant ASC, t.costume ASC")
	List<PokemonEntity> findAll();

	@Query("SELECT t FROM PokemonEntity t WHERE t.shiny=true ORDER BY t.dexid ASC, t.variant ASC, t.costume ASC")
	List<PokemonEntity> findAllShiny();
	
	@Query("SELECT t FROM PokemonEntity t WHERE t.shadow=true AND t.shiny=false ORDER BY t.dexid ASC, t.variant ASC, t.costume ASC")
	...
	@Query("SELECT t FROM PokemonEntity t WHERE t.shiny=true AND t.id NOT IN (SELECT t1.pokemonid FROM UserShinyPokemonEntity t1 "
			+ "WHERE t1.userid=?1) ORDER BY t.dexid ASC, t.variant ASC, t.costume ASC")
	List<PokemonEntity> findAllShinyForUser(Long userId);
	
	@Query("SELECT t FROM PokemonEntity t WHERE t.shadow=true AND t.shiny=false AND t.id NOT IN (SELECT t1.pokemonid FROM UserShadowPokemonEntity t1 "
			+ "WHERE t1.userid=?1) ORDER BY t.dexid ASC, t.variant ASC, t.costume ASC")
	...
	@Query("SELECT t FROM PokemonEntity t WHERE t.shiny=false ORDER BY t.dexid ASC, t.variant ASC, t.costume ASC")
